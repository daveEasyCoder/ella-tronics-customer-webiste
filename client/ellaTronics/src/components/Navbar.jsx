        ...
        <div className="text-sm text-gray-900  bg-red-500 w-full">
            <nav className="relative h-16 md:h-17.5 flex items-center justify-between px-6 py-4 bg-white shadow-md transition-all">
                ...
                <div className={`
                    ...
                    md:hidden z-50
                `}>
                    <div className="flex pt-6 flex-col space-y-2 p-6">
...

                        {
                            user && user?.role === 'admin' ?
                                <>
                                    ...
                                    <button onClick={handleLogout} className='self-start p-3 ml-1 font-medium text-lg rounded text-blue-600 flex items-center gap-2'> <LogOut size={20} /> <span>Logout</span></button>
                                </>
                                : user && user?.email ?
                                    <button onClick={handleLogout} className='self-start p-3 ml-1 font-medium text-lg rounded text-blue-600 flex items-center gap-2'> <LogOut size={20} /> <span>Logout</span></button> :
                                    <Link onClick={closeMenu} to="/user-login" className="flex self-start ml-3 border rounded-lg px-4 py-2 items-center space-x-2  text-blue-600 transition-colors">
                                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0zm6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                        </svg>
                                        <span>Get Started</span>
